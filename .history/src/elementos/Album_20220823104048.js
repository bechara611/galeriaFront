 ...
 useEffect(()=>{
  async function Metodo(){
    const respuesta = await GetObtenerImagenes()
      ...
      console.log(respuesta.response.data.errors[0].msg)
      if(respuesta.response.data.errors[0].msg){
        cambiarEstadoAlerta(true);
         cambiarTipoMensaje({tipo:'error',mensaje:respuesta.response.data.errors[0].msg})
